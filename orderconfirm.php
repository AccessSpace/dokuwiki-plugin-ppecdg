...
if ( $PaymentOption == "PayPal" )
{
	...
	if( $ack == "SUCCESS" || $ack == "SUCCESSWITHWARNING" )
	{
		 ...
		 * (ex: start download, start streaming, Add coins to the game.. etc)
		 ...
		 '********************************************************************************************************************
		 */
		 // DONE : save all of resArray as a page
		 $sLogNS = $oHelper->getConf('transactionlogns');
		 $sSep = ($conf['useslash']>0?'/':':'); 
		 $sLogID = cleanID($sLogNS.$sSep.$resArray["PAYMENTINFO_0_ORDERTIME"]);
		 ...
		 $sLog .= '<code>';
		 foreach($resArray as $sKey => $sValue)
		 {
		   $sLog .= $sKey." = ".$sValue."\n";
		 }
		 $sLog .= '</code>';
      
 		 saveWikiText($sLogID, $sLog, 'PayPal Order Confirmation Log');
    
     
		 // TODO : run different code depending on the parts
	...
	//	$exchangeRate		= $resArray["PAYMENTINFO_0_EXCHANGERATE"];  // Exchange rate if a currency conversion occurred. Relevant only if your are billing in their non-primary currency. If the customer chooses to pay with a currency other than the non-primary currency, the conversion occurs in the customer's account.
		...
		require_once('user_success_funcs.php');
		 
     $aSuccessURLs = array();
		 // TODO : take item name and parse into parts
		 foreach($items as $aItem)
		 {
		   $aNameParts = explode($sSep, $aItem['name']);
		   $sFunc = trim(strtolower(str_replace(array(' ',':','/'), '_',  $aNameParts[0])));
		   if(function_exists($sFunc))
		   {
		     $aParams = array($aItem['name'], $aNameParts, $aItem['amt'], $aItem['qty'],$res, $esArray);
		     $aSuccessURLs[] = call_user_func_array ( $sFunc , $aParams);
		   }
		 }
		header("Location: ".$aSuccessURLs[0]);
  ...
  <head>
     <title></title>
	   <script>
	     ...
	     */
	   if(window.opener){
	     window.opener.location.replace("<?php echo $aSuccessURLs[0];?>");
	     //window.location.href = "<?php echo $aSuccessURLs[0];?>";
	     //window.location.replace("<?php echo $aSuccessURLs[0];?>");
       //window.close();
     }
     else
     {
         ...
         if(top.dg.isOpen() == true){
           ...
           top.dg.closeFlow();
           top.location.href = "<?php echo $aSuccessURLs[0];?>";
         }
     }
     */
...
If this page does not redirect <a target="top" href="<?php echo $aSuccessURLs[0];?>">Click Here</a>
</body>
</html>
		<?php
	}
